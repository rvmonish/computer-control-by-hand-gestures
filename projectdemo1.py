...
while(1):        
    ...
    if bgCap==1:
        ...
        if z==1:
            ...
            _, th1 = cv2.threshold(vi,110, 255, cv2.THRESH_BINARY)
            cv2.imshow("thr10",th1)
            _, cs, hierarchy= cv2.findContours(th1,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
            ...
            for i in range(len(cs)):
                ...
                d1 = np.zeros(img.shape, np.uint8)
                cv2.drawContours(d1, [ct], 0, (0, 255, 0), 3)
                cv2.drawContours(d1, [hul], 0, (0, 0, 255), 3)
        ...
        if area>5:
            ...
            cv2.rectangle(frame,(a,b),(a+w,b+h),(0,0,255),0)
            #print(a,b)
            if a!=0 and b!=0:
                ox=240
                nx=1366
        ...
        #double click        
        if area>10 and p==0:
            l_ylw = np.array([20,90,100])
            u_ylw = np.array([50,255,255])
            ...
            for i in range(len(cnt_click)):
                ...
                #print("yellow area",area)
                    ...
                    cc=cnt_click[i]                    
            if area_click>10 and area_click<70:
        ...
        if p==1:    #panning 
            ...
            _, thr_pan = cv2.threshold(mask_pan,100, 255, cv2.THRESH_BINARY)
            _, cnt_pan, hierarchy = cv2.findContours(thr_pan,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
            max_area_ylw=-1
            area_ylw=0
            i=0
            tx1=0
            ...
            if area_ylw>10:
                ...
                if len1>5:
                    for j in range(len1-4,len1):
                        tx1=0
                        tx2=0
                        if(ply[j-1]<=ply[j]+3 or ply[j-1]>ply[j]-3):
                            for i in range(1,len(plx)-1):
                                #print("l1",plx[i],plx[i+1],ply[j])
                                if(plx[i+1]>plx[i]) and tx1<10:
                                    tx1=tx1+1
                                #else: tx1=0   
                        ...
                        if(plx[j-1]<=plx[j]+3 or plx[j-1]>plx[j]-3):
                ...
                if r1>2:
                    ...
                    pyautogui.click()
                    pyautogui.scroll(-15*d11)
                    d11=0
                elif u1>2:
                    pyautogui.click()
                    pyautogui.scroll(15*u1)
                    u1=0    """
